...
const Dashboard: React.FC<DashboardProps> = ({ cameras, onTryDemo, onViewEvents, onDeleteCamera }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-white relative" onClick={() => setOpenSettingsId(null)}>
            {/* Header */}
            ...
            <div className="flex-1 px-4 pt-4 z-20 space-y-4 overflow-y-auto pb-20 scrollbar-hide">
                ...
                {cameras.map((camera) => (
                    <div key={camera.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 relative">
                            ...
                            {camera.id !== 'cam-01' && (
                                <div className="relative">
                                    ...
                                    {openSettingsId === camera.id && (
                                        <div className="absolute top-6 right-0 bg-white border border-gray-100 shadow-lg rounded-xl py-1 z-50 min-w-[80px] animate-in fade-in zoom-in-95 duration-200">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    onDeleteCamera(camera.id);
                                                }}
                                                className="w-full text-center px-3 py-1 text-sm text-red-500 font-bold hover:bg-red-50"
                                            >
                                                Delete
                                            </button>
                                        </div>
                        ...
                        <div className={`aspect-video rounded-lg flex flex-col items-center justify-center gap-1 mb-2 relative overflow-hidden ${camera.status === 'online' ? 'bg-primary-950' : 'bg-[#D9D9D9]'}`}>
                            {camera.status === 'online' ? (
                                ...
                                <div className="text-center">
                                    <p className="font-bold text-sm text-[#8F9197]">No connection</p>
                                    <p className="text-[10px] text-[#8F9197]">This function is not available.</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center px-1 mb-2">
                            {/* Left: Date (Only if online, empty div otherwise to maintain layout if needed, or justify-between logic) - Using flex-1 to center middle */}
                            ...
                            <div className="flex justify-center flex-1">
                                ...
                                >
                                    <Folder className={`w-4 h-4 ${camera.status === 'offline' ? 'text-gray-300' : 'text-[#0D9488]'}`} />
                                    <span className="text-sm">Events</span>
                                </button>
                            </div>

                            {/* Right: Source */}
                            <span className="text-[10px] text-gray-400 lowercase flex-1 text-right">{camera.source}</span>
                        </div>
                    </div>
                ))}

                {/* Fallback Camera 1 if list is empty (though App should provide it) */}
                {cameras.length === 0 && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
                        <div className="flex justify-between items-center px-2 mb-2">
                            <span className="text-[#0D9488] font-bold text-sm">Camera 1</span>
                        </div>
                        <div className="bg-[#D9D9D9] aspect-video rounded-lg flex flex-col items-center justify-center gap-1 mb-2">
                            <p className="font-bold text-sm text-[#8F9197]">No connection</p>
                            <p className="text-[10px] text-[#8F9197]">This function is not available.</p>
                        </div>
                        <div className="flex items-center px-1 mb-2">
                            ...
                            <div className="flex justify-center flex-1">
                                <button
                                    disabled={true}
                                    className="flex items-center gap-1 text-gray-300 cursor-not-allowed opacity-50"
                                >
