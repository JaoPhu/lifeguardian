...
const SimulationRunning: React.FC<SimulationRunningProps> = ({ config, onStop, onEventAdded }) => {
    ...
    // Auto-Event Trigger code - kept same mostly
    useEffect(() => {
        if (!hasTriggeredEvent && config.eventType === 'sitting') {
            const timer = setTimeout(() => {
                setStickmanPosture('sitting');
                setHasTriggeredEvent(true);
            }, 1000);
            return () => clearTimeout(timer);
        } else if (!hasTriggeredEvent && config.eventType) {
            const timer = setTimeout(() => {
                setStickmanPosture(config.eventType as any);
                setHasTriggeredEvent(true);
                const type = config.eventType || 'sitting';
                const isCritical = type === 'falling';
    ...
    }, [config.eventType, hasTriggeredEvent, onEventAdded]); // Removed currentTime dep to avoid loops, unlikely to change firing time
